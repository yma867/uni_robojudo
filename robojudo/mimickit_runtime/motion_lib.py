...
class MotionLib:
    ...
    def sample_time(self, motion_ids, truncate_time=None):
        ...
        if truncate_time is not None:
            if truncate_time < 0.0:
                raise ValueError("truncate_time must be >= 0")
    ...
    def _load_motion_pkl(self, motion_file):
        ...
        for f in range(num_motion_files):
            ...
            joint_vel = torch.zeros_like(joint_rot[..., 0:3])
            joint_rot = torch_util.quat_pos(joint_rot)
            joint_drot = torch_util.quat_diff(joint_rot[:-1, ...], joint_rot[1:, ...])
            joint_vel[:-1, ...] = fps * torch_util.quat_to_exp_map(joint_drot)
            joint_vel[-1, ...] = joint_vel[-2, ...]

            dof_vel = self._kin_char_model.rot_to_dof(joint_vel)
        ...
        self._motion_num_frames = torch.tensor(self._motion_num_frames, dtype=torch.int64, device=self._device)
        self._motion_lengths = torch.tensor(self._motion_lengths, dtype=torch.float32, device=self._device)
        self._motion_loop_modes = torch.tensor(self._motion_loop_modes, dtype=torch.int64, device=self._device)
        ...
        self._build_motion_index_map()

    def _build_motion_index_map(self):
        num_motions = self.get_num_motions()
        self._motion_start_idx = torch.zeros(num_motions, dtype=torch.int64, device=self._device)
        for m in range(1, num_motions):
            prev_len = self._motion_num_frames[m - 1]
            self._motion_start_idx[m] = self._motion_start_idx[m - 1] + prev_len
    ...
    def _fetch_motion_files(self, motion_file):
        ...
        if ext == ".yaml":
            ...
            for motion_entry in motion_list:
                ...
                if curr_weight < 0:
                    raise ValueError("motion weight must be >= 0")
